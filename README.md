# Balatro Parallel Update
Status: $${\\color{green}Up‎ to‎ Date}$$

Welcome to Parallel Update! You have been selected to play a BRAND NEW UPDATE before anyone else. There will be a few changes this update, but some new faces to meet along the way.
Balatro Parallel Update (Formally Known as Community Update) is an UNOFFICIAL FAN-UPDATE to the game with no input by LocalThunk (So crashes should not be sent to them but to here instead). If you wish to work in some way on the project, either through playtests, ideas, etc, please check the balatro discord thread. This will have every addition and major change included.

# 1.A.0 Changelog

- Changed 8 Ball - Now 1 in 3, was 1 in 4
- Changed Green Joker
    - Now +2 per play, -3 per discard, was +1/-1
    - Now Uncommon, was Common
    - Now Costs $5, was $4
    - Changed Art
- Changed Madness
    - Now X0.75, was X0.5
    - Now Rare, was Uncommon
- Changed Square Joker 
    - Now +9 Chips, was +4.
    - Changed Art
- Changed Mystic Summit's Art
- Changed Lucky Cat - Now X0.2 Mult, was x0.25
- Changed Hanging Chad
    - Now Uncommon, was Common
    - Now Costs $5, was $4
- Changed Mail-in Rebate - Now $3, was $5
- Added 15 New Jokers
    - Middle of the Pack
    - Sticker Sheet
    - Querent
    - Coloring Page
    - Superstition
    - Ecstatic Joker
    - Sinister Joker
    - The Quarrel
    - Joker Proxy 
    - Office Cubicle
    - The Scales
    - Scryer
    - Fortune Cookie
    - Hat Trick
    - Quin
- Added Silver Seal
- Added Replicate
- Added Pelter & Pelter Flush (Secret Hand)
- Added Gilded Edition
    - Added Gilded Edition to on Hone, Glow-Up, Aura, and Wheel of Fortune
    - Added Gilded Tag 

- Added Dagon - Levels up Pelter
- Added Amateru - Levels up Pelter Flush
- 2 New Vouchers
- 2 New Challenges
